...
from melati.cmds.init_funcs import check_keys
from melati.consensus.block_rewards import calculate_base_farmer_reward
from melati.pools.pool_wallet import PoolWallet
from melati.pools.pool_wallet_info import create_pool_state, FARMING_TO_POOL, PoolWalletInfo, PoolState
...
from melati.util.keychain import bytes_to_mnemonic, generate_mnemonic
from melati.util.path import path_from_root
from melati.util.ws_message import WsRpcMessage, create_payload_dict
...
from melati.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_pool_sk, master_sk_to_wallet_sk
...
from melati.wallet.util.backup_utils import download_backup, get_backup_info, upload_backup
from melati.wallet.util.trade_utils import trade_record_to_dict
...
from melati.wallet.wallet_node import WalletNode
...
class WalletRpcApi:
    def __init__(self, wallet_node: WalletNode):
        ...
        self.service_name = "melati_wallet"
    ...
    async def _state_changed(self, *args) -> List[WsRpcMessage]:
        ...
        return [create_payload_dict("state_changed", data, "melati_wallet", "wallet_ui")]
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xmx_target_address")
        pool_target = config["pool"].get("xmx_target_address")
        found_farmer = False
        found_pool = False
    ...
    async def create_new_wallet(self, request: Dict):
        ...
        if request["wallet_type"] == "cc_wallet":
            ...
            if request["mode"] == "new":
                ...
                from melati.pools.pool_wallet_info import initial_pool_state_from_dict
