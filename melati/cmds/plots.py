...
def show_plots(root_path: Path):
    from melati.plotting.plot_tools import get_plot_directories

    print("Directories where plots are being searched for:")
    print("Note that subdirectories must be added manually")
    print(
        "Add with 'melati plots add -d [dir]' and remove with"
        + " 'melati plots remove -d [dir]'"
        + " Scan and check plots with 'melati plots check'"
    )
    print()
...
def plots_cmd(ctx: click.Context):
    ...
    if not root_path.is_dir():
        raise RuntimeError("Please initialize (or migrate) your config directory with 'melati init'")
...
def check_cmd(
    ...
    from melati.plotting.check_plots import check_plots
...
def add_cmd(ctx: click.Context, final_dir: str):
    from melati.plotting.plot_tools import add_plot_directory
...
def remove_cmd(ctx: click.Context, final_dir: str):
    from melati.plotting.plot_tools import remove_plot_directory
