...
@wallet_cmd.command("send", short_help="Send melati to another wallet")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option("-f", "--fingerprint", help="Set the fingerprint to specify which wallet to use", type=int)
@click.option("-i", "--id", help="Id of the wallet to use", type=int, default=1, show_default=True, required=True)
@click.option("-a", "--amount", help="How much melati to send, in XMX", type=str, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in XMX",
    type=str,
    default="0",
    show_default=True,
    required=True,
)
@click.option("-t", "--address", help="Address to send the XMX", type=str, required=True)
@click.option(
...
@click.option("-a", "--amount", help="How much melati to destroy to create this coloured coin, in TXMX/XMX", type=str)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in XMX",
...
def create_coloured_coin_cmd(wallet_rpc_port: int, colour_id: str, amount: str, fee: str, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(Decimal(fee) * units["melati"]))
    ...
    if colour_id:
        ...
        data["mode"] = "new"
        final_amount = uint64(int(Decimal(amount) * units["melati"]))
...
@click.option("-l", "--limit", help="Spendable amount per interval, in TXMX/XMX.", type=str, required=True)
@click.option("-a", "--amount", help="Amount for initial coin. in TXMX/XMX.", type=str, required=True)
@click.option(
    "-m",
    "--fee",
    help="Set the fees for the transaction, in XMX",
...
def create_rate_limited_admin_cmd(
    ...
    final_fee = uint64(int(Decimal(fee) * units["melati"]))
    final_amount = uint64(int(Decimal(amount) * units["melati"]))
    final_limit = uint64(int(Decimal(limit) * units["melati"]))
    data: Dict[str, Any] = {
        "fee": final_fee,
        "amount": final_amount,
        "pubkey": public_key,
