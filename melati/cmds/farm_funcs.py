...
from melati.cmds.units import units
...
from melati.rpc.wallet_rpc_client import WalletRpcClient
...
from melati.util.misc import format_bytes
from melati.util.misc import format_minutes
from melati.util.network import is_localhost
...
async def summary(
    ...
    if amounts is not None:
        print(f"Total melati farmed: {amounts['farmed_amount'] / units['melati']}")
        print(f"User transaction fees: {amounts['fee_amount'] / units['melati']}")
        print(f"Block rewards: {(amounts['farmer_reward_amount'] + amounts['pool_reward_amount']) / units['melati']}")
    ...
    if amounts is None:
        if wallet_not_running:
            print("For details on farmed rewards and fees you should run 'melati start wallet' and 'melati wallet show'")
        elif wallet_not_ready:
            print("For details on farmed rewards and fees you should run 'melati wallet show'")
    else:
        print("Note: log into your key using 'melati wallet show' to see rewards for each key")
