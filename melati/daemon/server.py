...
from melati.cmds.init_funcs import melati_init
from melati.daemon.windows_signal import kill
from melati.server.server import ssl_context_for_root, ssl_context_for_server
...
from melati.util.json_util import dict_to_json_str
from melati.util.path import mkdir
from melati.util.service_groups import validate_service
from melati.util.setproctitle import setproctitle
from melati.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Melati.")
    quit()
...
service_plotter = "melati plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "melati": "melati",
        "melati_wallet": "start_wallet",
        "melati_full_node": "start_full_node",
        "melati_harvester": "start_harvester",
        "melati_farmer": "start_farmer",
        "melati_introducer": "start_introducer",
        "melati_timelord": "start_timelord",
        "melati_timelord_launcher": "timelord_launcher",
        "melati_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("melati daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered MELATI_ROOT
    os.environ["MELATI_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with MELATI_ROOT: {os.environ['MELATI_ROOT']}")
...
async def async_run_daemon(root_path: Path) -> int:
    melati_init(root_path)
    config = load_config(root_path, "config.yaml")
    setproctitle("melati_daemon")
