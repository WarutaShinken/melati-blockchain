...
import melati.server.ws_connection as ws  # lgtm [py/import-and-import-from]
from melati.consensus.block_creation import unfinished_block_to_full_block
from melati.consensus.block_record import BlockRecord
from melati.consensus.blockchain import Blockchain, ReceiveBlockResult
from melati.consensus.constants import ConsensusConstants
from melati.consensus.difficulty_adjustment import get_next_sub_slot_iters_and_difficulty
from melati.consensus.make_sub_epoch_summary import next_sub_epoch_summary
from melati.consensus.multiprocess_validation import PreValidationResult
from melati.consensus.pot_iterations import calculate_sp_iters
from melati.full_node.block_store import BlockStore
from melati.full_node.bundle_tools import detect_potential_template_generator
from melati.full_node.coin_store import CoinStore
from melati.full_node.full_node_store import FullNodeStore
from melati.full_node.mempool_manager import MempoolManager
from melati.full_node.signage_point import SignagePoint
from melati.full_node.sync_store import SyncStore
from melati.full_node.weight_proof import WeightProofHandler
from melati.protocols import farmer_protocol, full_node_protocol, timelord_protocol, wallet_protocol
from melati.protocols.full_node_protocol import (
    RejectBlocks,
    RequestBlocks,
    RespondBlock,
    RespondBlocks,
...
from melati.server.node_discovery import FullNodePeers
...
from melati.types.blockchain_format.sub_epoch_summary import SubEpochSummary
from melati.types.blockchain_format.vdf import CompressibleVDFField, VDFInfo, VDFProof
from melati.types.end_of_slot_bundle import EndOfSubSlotBundle
from melati.types.full_block import FullBlock
from melati.types.header_block import HeaderBlock
from melati.types.mempool_inclusion_status import MempoolInclusionStatus
from melati.types.spend_bundle import SpendBundle
from melati.types.unfinished_block import UnfinishedBlock
from melati.util.bech32m import encode_puzzle_hash
from melati.util.db_wrapper import DBWrapper
from melati.util.errors import ConsensusError, Err
from melati.util.ints import uint8, uint32, uint64, uint128
from melati.util.path import mkdir, path_from_root
from melati.util.safe_cancel_task import cancel_task_safe
from melati.util.profiler import profile_task
...
class FullNode:
    ...
    async def short_sync_batch(self, peer: ws.WSMelatiConnection, start_height: uint32, target_height: uint32) -> bool:
        """
    ...
    async def short_sync_backtrack(
        self, peer: ws.WSMelatiConnection, peak_height: uint32, target_height: uint32, target_unf_hash: bytes32
    ):
        """
    ...
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSMelatiConnection):
        """
    ...
    async def send_peak_to_timelords(
        self, peak_block: Optional[FullBlock] = None, peer: Optional[ws.WSMelatiConnection] = None
    ):
        """
    ...
    async def on_connect(self, connection: ws.WSMelatiConnection):
        """
    ...
    async def peak_post_processing(
        self, block: FullBlock, record: BlockRecord, fork_height: uint32, peer: Optional[ws.WSMelatiConnection]
    ):
        """
    ...
    async def respond_block(
        ...
        peer: Optional[ws.WSMelatiConnection] = None,
    ) -> Optional[Message]:
        """
    ...
    async def respond_unfinished_block(
        ...
        peer: Optional[ws.WSMelatiConnection],
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, timelord_peer: Optional[ws.WSMelatiConnection] = None
    ) -> Optional[Message]:
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSMelatiConnection
    ...
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSMelatiConnection):
    ...
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSMelatiConnection):
        header_block = await self.blockchain.get_header_block_by_height(
            request.height, request.header_hash, tx_filter=False
        )
        if header_block is None:
    ...
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSMelatiConnection):
        field_vdf = CompressibleVDFField(int(request.field_vdf))
        if not await self._can_accept_compact_proof(
            request.vdf_info, request.vdf_proof, request.height, request.header_hash, field_vdf
        ):
