...
from melati.wallet.cc_wallet import cc_utils
from melati.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from melati.wallet.cc_wallet.cc_wallet import CCWallet
from melati.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from melati.wallet.trade_record import TradeRecord
from melati.wallet.trading.trade_status import TradeStatus
from melati.wallet.trading.trade_store import TradeStore
from melati.wallet.transaction_record import TransactionRecord
from melati.wallet.util.trade_utils import (
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_melati(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_spends:
            ...
            if r:
                ...
                # standard melati coin
                unspent = await self.wallet_state_manager.get_spendable_coins_for_wallet(1)
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if melati_discrepancy is None:
                    melati_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    melati_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        melati_spend_bundle: Optional[SpendBundle] = None
        if melati_discrepancy is not None:
            melati_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_melati(
                melati_discrepancy, []
            )
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if melati_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = melati_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have melati available",
                        )
        ...
        now = uint64(int(time.time()))
        if melati_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, melati_spend_bundle])
            if melati_discrepancy < 0:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
                    created_at_time=now,
                    to_puzzle_hash=token_bytes(),
                    amount=uint64(abs(melati_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=melati_spend_bundle,
                    additions=melati_spend_bundle.additions(),
                    removals=melati_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=melati_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
