...
from melati.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH
from melati.protocols import wallet_protocol
from melati.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from melati.protocols.protocol_message_types import ProtocolMessageTypes
from melati.protocols.wallet_protocol import (
...
from melati.util.errors import Err, ValidationError
from melati.util.ints import uint32, uint128
from melati.util.keychain import Keychain
from melati.util.lru_cache import LRUCache
from melati.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
...
from melati.wallet.settings.settings_objects import BackupInitialized
...
from melati.wallet.wallet_blockchain import ReceiveBlockResult
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]) -> Optional[PrivateKey]:
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'melati keys' program.")
            return None
