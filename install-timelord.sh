...
THE_PATH=$(python -c 'import pkg_resources; print( pkg_resources.get_distribution("melativdf").location)' 2>/dev/null)/vdf_client
CHIAVDF_VERSION=$(python -c 'from setup import dependencies; t = [_ for _ in dependencies if _.startswith("melativdf")][0]; print(t)')
...
if [ -e "$THE_PATH" ]; then
	...
	if [ -e venv/bin/python ] && test $UBUNTU_DEBIAN; then
		echo "Installing melativdf from source on Ubuntu/Debian"
		...
		echo "Installing melativdf from source on RedHat/CentOS/Fedora"
		# Install remaining needed development tools - assumes venv and prior run of install.sh
		...
		echo "Installing melativdf requirement boost for MacOS."
		brew install boost
		echo "Installing melativdf from source."
