...
from melati.util.errors import Err
from melati.full_node.generator import setup_generator_args
from melati.full_node.mempool_check_conditions import GENERATOR_MOD
from melati.plotting.create_plots import create_plots
...
from melati.consensus.blockchain_interface import BlockchainInterface
from melati.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from melati.consensus.deficit import calculate_deficit
from melati.consensus.full_block_to_block_record import block_to_block_record
...
from melati.consensus.pot_iterations import (
    calculate_ip_iters,
...
from melati.consensus.vdf_info_computation import get_signage_point_vdf_info
from melati.full_node.signage_point import SignagePoint
from melati.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from melati.types.blockchain_format.foliage import Foliage, FoliageBlockData, FoliageTransactionBlock, TransactionsInfo
...
from melati.types.blockchain_format.reward_chain_block import RewardChainBlockUnfinished
from melati.types.blockchain_format.sized_bytes import bytes32
from melati.types.blockchain_format.slots import (
...
from melati.types.blockchain_format.vdf import VDFInfo, VDFProof
...
from melati.types.generator_types import BlockGenerator, CompressorArg
from melati.types.spend_bundle import SpendBundle
from melati.types.unfinished_block import UnfinishedBlock
from melati.types.name_puzzle_condition import NPC
from melati.util.bech32m import encode_puzzle_hash
from melati.util.block_cache import BlockCache
from melati.util.condition_tools import ConditionOpcode, conditions_by_opcode
from melati.util.config import load_config, save_config
from melati.util.hash import std_hash
from melati.util.ints import uint8, uint16, uint32, uint64, uint128
from melati.util.keychain import Keychain, bytes_to_mnemonic
from melati.util.merkle_set import MerkleSet
from melati.util.prev_transaction_block import get_prev_transaction_block
from melati.util.path import mkdir
from melati.util.vdf_prover import get_vdf_info_and_proof
from tests.wallet_tools import WalletTool
from melati.wallet.derive_keys import (
...
class BlockTools:
    ...
    def __init__(
        ...
        create_default_melati_config(root_path)
        ...
        if len(self.pool_pubkeys) == 0 or len(self.farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `melati generate keys`")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("melati_ROOT", "~/.melati/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
