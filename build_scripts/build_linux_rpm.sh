...
# sets the version for melati-blockchain in package.json
cp package.json package.json.orig
jq --arg VER "$MELATI_INSTALLER_VERSION" '.version=$VER' package.json > temp.json && mv temp.json package.json
...
if [ "$REDHAT_PLATFORM" = "x86_64" ]; then
	echo "Create melati-blockchain-$MELATI_INSTALLER_VERSION.rpm"

	# shellcheck disable=SC2046
	NODE_ROOT="$(dirname $(dirname $(which node)))"

	# Disables build links from the generated rpm so that we dont conflict with other packages. See https://github.com/Melati-Network/melati-blockchain/issues/3846
  ...
  --arch "$REDHAT_PLATFORM" --options.version $MELATI_INSTALLER_VERSION \
  --license ../LICENSE
  LAST_EXIT_CODE=$?
  if [ "$LAST_EXIT_CODE" -ne 0 ]; then
